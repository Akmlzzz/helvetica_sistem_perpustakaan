    ...
    <div x-data="{
        ...
        <div class="relative w-full h-full">
            @foreach($banners as $index => $banner)
                ...
                <div x-show="activeSlide === {{ $index }}"
                    ...
                    <!-- Bottom Fade -->
                    <div
                        class="absolute bottom-0 left-0 w-full h-32 bg-linear-to-t from-[#F4F7FE] dark:from-[#1A222C] to-transparent z-20">
                    </div>

                    <!-- Layer 2: Character (Right Side) -->
                    @if($banner->char_img)
                        <div
                            class="absolute bottom-0 right-0 sm:right-4 md:right-10 z-10 h-[70%] sm:h-[80%] md:h-[90%] w-auto flex items-end">
                    ...
                    <div
                        class="absolute top-1/2 -translate-y-1/2 left-4 sm:left-6 md:left-12 z-30 max-w-[70%] sm:max-w-[60%] md:max-w-lg text-white">
                        ...
                        @if($banner->tags)
                            ...
                            <div class="flex flex-wrap gap-1.5 sm:gap-2 mb-2 sm:mb-4">
                                @foreach($tagsArray as $tag)
                                    <span
                                        class="px-2 py-0.5 sm:px-3 sm:py-1 text-[10px] sm:text-xs font-semibold bg-white/20 backdrop-blur-md rounded-full border border-white/30 text-[#084734]">
                                        {{ trim($tag) }}
                                    </span>
                                @endforeach
                            </div>
        ...
        <div class="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex space-x-2 z-40">
            <template x-for="i in slides">
                <button @click="goTo(i-1)" class="w-2.5 h-2.5 rounded-full transition-all duration-300"
