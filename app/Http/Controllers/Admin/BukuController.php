...
class BukuController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->filled('kategori')) {
            $query->whereHas('kategori', fn($q) => $q->where('kategori.id_kategori', $request->kategori));
        }
        ...
        $this->applySorting($query, $request->sort, 'dibuat_pada');
    ...
    {
        $request->validate($this->bukuValidationRules());
        ...
        if ($request->hasFile('sampul')) {
            $this->deleteOldSampul($buku->sampul);
    ...
    public function show($id)
    {
        ...
        $relatedBooks = Buku::whereHas('kategori', fn($q) =>
            $q->whereIn('kategori.id_kategori', $buku->kategori->pluck('id_kategori'))
        )->where('id_buku', '!=', $buku->id_buku)->limit(6)->get();

        $seriesVolumes = $buku->id_series
            ? Buku::where('id_series', $buku->id_series)->orderBy('nomor_volume')->get()
            : null;
        ...
        try {
            foreach ($request->items as $index => $itemData) {
                $data = [
                    ...
                    'sampul'        => $request->hasFile("items.$index.sampul")
                        ? $request->file("items.$index.sampul")->store('sampul', 'public')
                        : null,
                ];
            ...
            return redirect()->route('admin.buku.index')
                ->with('success', count($request->items) . ' buku berhasil ditambahkan sekaligus!');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()
                ->with('error', 'Gagal menambahkan batch buku: ' . $e->getMessage())
                ->withInput();
        }
    }
    ...
    private function bukuValidationRules(): array
    {
        return [
            'judul_buku'    => 'required|string|max:255',
            'isbn'          => 'nullable|string|max:20|regex:/^[0-9\-\s]+$/',
    ...
    private function deleteOldSampul(?string $sampul): void
    {
        if ($sampul && Storage::disk('public')->exists($sampul)) {
            Storage::disk('public')->delete($sampul);
        }
    }

    private function applySorting($query, ?string $sort, string $defaultColumn): void
    {
        match ($sort) {
            'terlama' => $query->orderBy($defaultColumn, 'asc'),
            'az'      => $query->orderBy('judul_buku', 'asc'),
            default   => $query->orderBy($defaultColumn, 'desc'),
        };
    }
}
